...
class WC_Price_Calculator_Product_Page {
	...
	private function add_price_html_filters() {
		...
		if ( SV_WC_Plugin_Compatibility::is_wc_version_gte_3_0() ) {
			add_filter( 'woocommerce_product_variation_get_price_html', array( $this, 'price_per_unit_html' ), 10, 2 );
		} else {
	...
	private function remove_price_html_filters() {
		...
		if ( SV_WC_Plugin_Compatibility::is_wc_version_gte_3_0() ) {
			remove_filter( 'woocommerce_product_variation_get_price_html', array( $this, 'price_per_unit_html' ), 10 );
		} else {
			...
			if ( $settings->is_quantity_calculator_enabled() ) {
				...
				// other product types
					...
					if ( $measurement && '' !== $price_html && $measurement->get_value() ) {
						...
						$original_price         = SV_WC_Product_Compatibility::get_prop( $product, 'price' );
						$original_regular_price = SV_WC_Product_Compatibility::get_prop( $product, 'regular_price' );
						$original_sale_price    = SV_WC_Product_Compatibility::get_prop( $product, 'sale_price' );

						// calculate the price per unit, then format it
						$product->price         = $original_price         / $measurement->get_value();
						$product->regular_price = $original_regular_price / $measurement->get_value();
						...
						if ( ! empty( $original_sale_price ) ) {
							$product->sale_price = $original_sale_price / $measurement->get_value();
						}
