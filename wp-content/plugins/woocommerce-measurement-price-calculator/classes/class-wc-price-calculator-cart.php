...
class WC_Price_Calculator_Cart {
	...
	public function set_order_item_meta( $item, $cart_item_key, $values ) {
		...
		if ( isset( $values['pricing_item_meta_data'] ) ) {
			...
			if ( isset( $values['pricing_item_meta_data']['_quantity'] ) && WC_Price_Calculator_Product::pricing_calculator_inventory_enabled( $item->get_product() ) ) {
				...
				add_filter( 'woocommerce_order_item_quantity', array( $this, 'set_actual_unit_quantity' ), 10, 3 );
			}
		}
	}
	 ...
	 * that order stock is reduced at checkout by the calculated amount.
	 *
	 * Note that this won't be run if order stocked is reduced outside
	 * of the checkout process, e.g. by a gateway that calls wc_reduce_order_stock()
	 * at a later date.
	 *
	 * @internal
	 *
	 * @since 3.11.0
	 * @param int $quantity quantity to reduce order stock by
	 * @param \WC_Order $order, unused
	 * @param \WC_Order_item_Product $item
	 * @return int
	 */
	public function set_actual_unit_quantity( $quantity, $order, $item ) {

		$measurement_data = $item->get_meta( '_measurement_data' );

		if ( ! empty( $measurement_data['_measurement_needed'] ) ) {
			$quantity = $measurement_data['_measurement_needed'];
		}
	...
	public function set_cart_contents_weight() {
		...
		foreach ( WC()->cart->get_cart() as $cart_item_key => $item ) {
			...
			$weight += $weight * $this->get_item_quantity( $item );
		}
