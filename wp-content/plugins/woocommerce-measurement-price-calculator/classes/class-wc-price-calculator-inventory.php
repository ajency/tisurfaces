...
class WC_Price_Calculator_Inventory {
	...
	public function get_measurement_stock_amount( $quantity, $cart_item_key = null ) {
		...
		// The quantity check ensues that the stock amount is only modified for the add to cart action, and not the WC_Cart::get_cart_from_session() method
		// which runs first in WC 3.0+
		if ( ! $this->pricing_stock_altered && isset( $_REQUEST['add-to-cart'] ) && is_numeric( $_REQUEST['add-to-cart'] ) && $quantity == $_REQUEST['quantity'] ) {

			// In WC 3.0+ getting the product inside of a method hooked into woocommerce_stock_amount causes an infinite filter loop,
			// so remove them prior to doing so.
			if ( SV_WC_Plugin_Compatibility::is_wc_version_gte_3_0() ) {
				remove_filter( 'woocommerce_stock_amount',           array( $this, 'get_measurement_stock_amount' ), 5 );
				remove_filter( 'woocommerce_stock_amount_cart_item', array( $this, 'get_measurement_stock_amount' ), 10 );
			}
