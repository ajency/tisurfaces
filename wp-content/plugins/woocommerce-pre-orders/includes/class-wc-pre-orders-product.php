...
class WC_Pre_Orders_Product {
	...
	public static function product_is_charged_upon_release( $product ) {
		...
		return 'upon_release' === get_post_meta( $product->get_id(), '_wc_pre_orders_when_to_charge', true );
	}
	...
	public static function product_is_charged_upfront( $product ) {
		...
		return 'upfront' === get_post_meta( $product->get_id(), '_wc_pre_orders_when_to_charge', true );
	}
	...
	public static function get_localized_availability_datetime_timestamp( $product ) {
		...
		if ( ! $product || ! $timestamp = get_post_meta( $product->get_id(), '_wc_pre_orders_availability_datetime', true ) ) {
			return 0;
		}
